...
exports.register = (server, options, next) => {
    ...
    const methodInjectHelper = (methodsFilename, methodPrefix, injectModuleValue, injectModuleKey) => {
        ...
        if (isFunction(injectModuleValue)) {
            server.method(methodName, injectModuleValue);
            isValid = true;
        }

        if (injectModuleValue.options && injectModuleValue.method && isFunction(injectModuleValue.method)) {
        ...
        return Async.each(castArray(options.methods), (injectItem, nextInjectItem) => {
            ...
            getFiles(injectItem, (err, files) => {
                ...
                return Async.each(files, (file, nextFile) => {
                    ...
                    if (isFunction(injectModule) || injectModule.options) {
